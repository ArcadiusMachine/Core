function KOTH:Player(playerID)
    ...
    if ids[Config.Player.Connection] == "" then
        print('No connection ID found.')
    end

    local uuid = Config.Player.Connection .. "ID"
    local res = MySQL.Async.fetchAll('SELECT * FROM users WHERE ' .. uuid ..'=@ID', { ['@ID'] = ids[Config.Player.Connection] })
    ...
    if res then
        ...
        MySQL.Async.execute('INSERT INTO users (steamID, discordID) VALUES (@steamID, @discordID)', {
            ['@steamID'] = ids.steam,
    ...
    return {
        ...
        Money = {
            ...
            add = function(self, amount)
                  ...
                  MySQL.Async.execute("UPDATE users SET cash = cash + @added WHERE ".. uuid .."=@ID", { ['@ID'] = ids[Config.Player.Connection], ['@added'] = amount})
                  return self.cash
            end,
            ...
            reset = function(self)
                  ...
                  MySQL.Async.execute("UPDATE users SET cash = 0 WHERE ".. uuid .."=@ID", { ['@ID'] = ids[Config.Player.Connection]})
                  return true
            end
        },
        Level = {
            ...
            addXP = function(self, amount)
                  ...
                  MySQL.Async.execute("UPDATE users SET xp = xp + @added WHERE ".. uuid .."=@ID", { ['@ID'] = ids[Config.Player.Connection], ['@added'] = amount})
                  return self.xp
            end,
            ...
            reset = function(self)
                  ...
                  MySQL.Async.execute("UPDATE users SET xp = 0 WHERE ".. uuid .."=@ID", { ['@ID'] = ids[Config.Player.Connection]})
                  return true
            end
        },
        Identifiers = ids,
        ...
        Weapons = {
            ...
            add = function(self, weapon)
                ...
                if hash ~= nil then
                    ...
                    MySQL.Async.execute("UPDATE users SET weapons = @str WHERE ".. uuid .."=@ID", { ['@str'] = str, ['@ID'] = ids[Config.Player.Connection]})
                    return true
                end
            ...
            reset = function(self)
                ...
                self.items = {}
                MySQL.Async.execute("UPDATE users SET weapons = '' WHERE ".. uuid .."=@ID", { ['@ID'] = ids[Config.Player.Connection] })
        ...
        Vehicles = {
            ...
            add = function(self, vehicle)
                ...
                if hash ~= nil then
                    ...
                    local str = table.join(self.items, ",")
                    MySQL.Async.execute("UPDATE users SET vehicles = @str WHERE ".. uuid .."=@ID", { ['@str'] = str, ['@ID'] = ids[Config.Player.Connection]})
                    return true
                end
            ...
            reset = function(self)
                ...
                self.items = {}
                MySQL.Async.execute("UPDATE users SET vehicles = '' WHERE ".. uuid .."=@ID", { ['@ID'] = ids[Config.Player.Connection] })
...
AddEventHandler("playerConnecting", function(name, setKickReason, deferrals)
    local src = source
    local player = KOTH:Player(tostring(src))
